...
class Decoder(tfkl.Layer):
    def __init__(
        self, act="relu", init="glorot_uniform", bias=1e-4, name="decoder", **kwargs
    ):
        super().__init__(name=name, **kwargs)
        self.act = act
        self.init = init
    ...
    def call(self, inputs):
        x = self.bn1(self.dec1(inputs))
        x = self.bn2(self.dec2(x))
        x = self.bn3(self.dec3(x))
        return x
...
class ShaRP(tfk.Model):
    ...
    def save_weights(self, export_path: str, *args, **kwargs):
        ...
        self.log_var_model.save_weights(
            os.path.join(export_path, "logvar"), *args, **kwargs
        )
    ...
    def load_weights(self, export_path: str, *args, **kwargs):
        ...
        self.log_var_model.load_weights(
            os.path.join(export_path, "logvar"), *args, **kwargs
        )
    ...
    def fit(
        ...
        **kwargs,
        ...
        if isinstance(X_train, tf.data.Dataset):
            ...
            X_res = np.concatenate((X_train, np.ones((X_train.shape[0],1))), axis=1)#
...
def compute_all_metrics(
    X, X_2d, D_high, D_low, y, X_inv=None, X_test=None, X_inv_test=None
):
...
def main():
    ...
    print(
        compute_all_metrics(
            X_train, X_i, D_high, D_low, y_train, sharp.inverse_transform(X_i)
        )
    )
    plt.show()
    ...
    diffs = (
        var_metrics.finite_differences(sharp, sample_points, eps=1e-2)
        .numpy()
        .sum(axis=1)
    )
    variances = np.exp(
        sharp.log_var_embedding(sharp.inverse_transform(sample_points)) * 0.5
    ).sum(axis=1)
    ...
    plt.subplot(2, 2, 3)
    plt.title(
        f"Finite Difference Gradient (min={diffs.min():.3g}, max={diffs.max():.3g})"
    )
