...
class SphericalSampling(SamplingLayer):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            latent_dim == 3
        ), "SphericalSampling currently only supports 3-D latent_dim."
        ...
        #         ),
        #         tfkl.Lambda(self._map_to_sphere),
        #         # tfkl.Lambda(lambda x: tf.nn.l2_normalize(x, axis=-1)),
        ...
        self.dense_mean = tfkl.Dense(
            ...
            bias_initializer=Constant(bias),
            activity_regularizer=regularizers.l1_l2(l1=l1_reg, l2=l2_reg),
            activation=lambda x: tf.nn.l2_normalize(x, epsilon=1e-5, axis=-1),
        )

        self.dense_concentration = tfk.Sequential(
            [
                tfkl.Dense(
                    1,
                    activation="softplus",
        ...
        # self.dense_concentration = tfk.Sequential(
        #     [tfkl.Dense(1), tfkl.Lambda(lambda x: x / 10.0), tfkl.Activation("exponential")]
        # )

        self.sampling = tfpl.DistributionLambda(
            make_distribution_fn=lambda t: tfd.VonMisesFisher(
                mean_direction=t[..., :3],
                concentration=t[..., 3],
            ...
            activity_regularizer=tfpl.KLDivergenceRegularizer(
                tfd.SphericalUniform(3), weight=0.01
            ),
        )
    ...
    def _sample(self, inputs, training=None):
        ...
        # mean = tf.math.mod(mean, 2 * np.pi) - np.pi
        # mean_directions = self._map_to_sphere(mean)
        # tf.print(tf.reduce_max(tf.linalg.norm(mean_directions, axis=-1)))
        # mean[:, 1] = tf.clip_by_value(mean[:, 1], -np.pi, np.pi)
        batch_norm = self.batch_norm(inputs)

        conc = self.dense_concentration(inputs)

        if training:
            reg_loss = 0.05 * tf.reduce_mean(
                0.000 * mean[:, 0] ** 2 + 0.005 * losses.log_cosh(conc[:, :1], 0.0)
            )
            # self.add_loss(reg_loss)
        return self.sampling(tf.concat([mean, batch_norm, conc], axis=-1))

    def _map_to_sphere(self, angles):
        # Mapping from angles to points on the unit sphere i.e. directions for VonMisesFisher.
        # Parametrizing the sampling operation with a direction + concentration is better than
        # sampling angles and then mapping them to the sphere, as it avoids corner cases like
        # the two poles of the sphere.
        tf.debugging.check_numerics(angles, "angles")
        tf.debugging.check_numerics(tf.math.sin(angles), "angles")
        tf.debugging.check_numerics(tf.math.cos(angles), "angles")
        return tf.stack(
            [
                tf.math.sin((angles[:, 0] + np.pi) / 2)
                * tf.math.cos(angles[:, 1] + np.pi),
                tf.math.sin((angles[:, 0] + np.pi) / 2)
                * tf.math.sin(angles[:, 1] + np.pi),
                tf.math.cos((angles[:, 0] + np.pi) / 2),
            ],
            axis=-1,
        )

    def _map_to_angles(self, cartesian):
        theta = cartesian[:, 2]
        # phi = tf.cond(
        #     theta <= 1e-8,
        #     lambda: tf.zeros_like(theta),
        #     tf.math.atan2(cartesian[:, 1], cartesian[:, 0] + 1e-8),
        # )
        phi = cartesian[:, 1] / (cartesian[:, 0] + 1)
        # phi = tf.math.sign(cartesian[:, 1]) * tf.math.acos(
        #     cartesian[:, 0]
        #     / (tf.math.sqrt(cartesian[:, 0] ** 2 + cartesian[:, 1] ** 2 + 1e-12))
        # )
        ...
        **kwargs,
        ...
        self.sampling = tfpl.DistributionLambda(
            make_distribution_fn=lambda t: tfd.MultivariateNormalDiag(
                ...
                scale_diag=tf.square(t[..., self.latent_dim :]),
            ),
...
class CenteredDiagonalNormalSampling(SamplingLayer):
    ...
    def call(self, inputs):
        ...
        self.add_loss(
            ...
            * tf.reduce_mean(
                tf.reduce_sum(
                    tfd.MultivariateNormalDiag(loc=0, scale_diag=scale).log_prob(
                        samples
                    )
...
class TriangleSampling(SamplingLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.triangle_corners = tf.convert_to_tensor(
            np.array([[0.0, 0.0], [0.5, np.sqrt(3) / 2], [1.0, 0.0]]).T,
            dtype=tf.float32,
        )
        ...
        self.dense_rot_angle = tfk.Sequential(
            [
                self.make_dense_param_layer(
                    1, activation="tanh", activity_regularizer=None
                ),
        ...
        self.dense_concentration = tfk.Sequential(
            [
                self.make_dense_param_layer(3, activation="softplus"),
                tfkl.Lambda(lambda x: x + 1),
            ]
        )
        self.sampling = tfpl.DistributionLambda(
    ...
    def make_rot_matrix(self, angle_radians: tf.Tensor) -> tf.Tensor:
        ...
        b = (
            tf.eye(2) * c[:, None, None]
            + tf.constant([[0.0, -1.0], [1.0, 0.0]]) * s[:, None, None]
        )
    ...
    def call(self, inputs):
        ...
        self.add_loss(
            ...
            * tf.reduce_mean(
                ...
                * tf.reduce_sum(
                    tfd.Dirichlet(concentration=concentration).log_prob(
                        barycentric_coords
                    )
                ...
                * tf.reduce_sum(tf.square(bias), axis=-1)
                + self.scale_loss_weight
                * tf.reduce_sum(tf.square(scale_factors - 1), axis=-1)
        ...
        return (
            tf.zeros_like(barycentric_coords),
            tf.zeros_like(barycentric_coords),
            samples,
        )
...
class LaplaceSampling(SamplingLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.dense_scale = self.make_dense_param_layer(
            self.latent_dim, activation="softplus"
        )
        self.sampling = tfpl.DistributionLambda(
...
class PolygonSampling(SamplingLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.dense_rot_angle = tfk.Sequential(
            [
                self.make_dense_param_layer(1, activation="tanh"),
                tfkl.Lambda(lambda x: x * np.pi),
            ]
        )
        ...
        self.polygon_vertices = tf.convert_to_tensor(
            polygon_vertices.T, dtype=tf.float32
        )
        self.sampling = tfpl.DistributionLambda(
            make_distribution_fn=lambda t: tfd.Dirichlet(concentration=t)
        )
...
class StarSampling(SamplingLayer):
    ...
    def call(self, inputs):
        ...
        coords = tf.concat(
            [radii * star_radii * tf.cos(angles), radii * star_radii * tf.sin(angles)],
            axis=-1,
        )
    ...
    def _star_radius(self, theta: float) -> float:
        ...
        den = tf.cos(
            tf.asin(tf.cos(self.n_points * theta)) + np.pi * 3 / (2 * self.n_points)
        )
        return num / den
