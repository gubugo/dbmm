...
def get_inv_proj_data(output_dir, _model, _inv_model, dataset_name, model_name, epochs):
    ...
    if model_name != "nninv":
        ...
        try:
            _inv_model = load(f'{output_dir}/{dataset_name}/{model_name}.joblib')
        except:
            _inv_model.fit(X_model_res, X_train)
            dump(_inv_model, f'{output_dir}/{dataset_name}/{model_name}.joblib')
...
if __name__ == "__main__":
    ...
    output_dir = "results_inverse"
    method = st.sidebar.selectbox("Inverse Projection Method", ("ssnp", "sharp"))
    dataset = "mnist"
